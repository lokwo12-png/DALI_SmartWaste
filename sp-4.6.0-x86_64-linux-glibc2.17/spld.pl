...
my $opt_config_default="/denis_user/local/sicstus4.6.0/bin/spconfig-4.6.0"; # [PM] 3.8.5 sprm 1678
...
my $opt_keep=0;
my $allow_shared_static=1; 	# [PM] 4.4.2 Support shared all-in-one, i.e. --shared --static. SPRM-20254.
my $opt_static=0;
...
my $config_file_basename="spconfig-4.6.0";
my $release_year="2020"; # [PM] 3.11.2
...
Copyright (C) $release_year, RISE Research Institutes of Sweden AB
...
Usage: spld [options] files...
    ...
    -S,--static                 Prefer static linking over dynamic.
                                ...
                                By default this creates an all-in-one executable that
                                contains all needed code and data except the system
                                libraries. Can be combined with --shared to create an
                                all-in-one shared library.
...
if ($opt_shared) {
  ...
  if ($opt_static && !$allow_shared_static) {
...
SWITCH: {
  ...
  if (/prolog/) {
    if (@prologfiles > 0 
	or (@savfiles > 0
	    # [PM] 4.4.2 Do not warn about sav files if they will become embedded
	    && !$opt_embed_sav_file)) {
...
if ($opt_shared) {
  ...
  # [PM] 4.4.2 Exe libs needed for --shared --static when embedding
  # runtime, because embedded runtime needs them, e.g. "-lpthread" or
  # "-framwork CoreFoundation".
  my @spld_shared_static_libs = ($opt_static && (not $opt_no_sprt)) ? @spld_exe_libs : ();
