...
Options for SICStus/FlatZinc:\n\
        ...
        -s\n\
                emit statistics\n\
        -t, -time (unsigned int) default: no cutoff\n\
                time (in ms) cutoff\n\
...
user:runtime_entry(start) :-
	...
	set_prolog_flag(gc_margin, 500000), % prevent excessive GC
...
parse_option([timeout(N)|Opt], Opt) --> ['-t',A], !,
	{atom_codes(A, Codes), number_codes(N, Codes)}.
...
parse_option(Opt, Opt) --> ['-f'], !.
parse_option(Opt, Opt) --> ['-p',A], !,
	{atom_codes(A, Codes), number_codes(_, Codes)}.
