...
:- queue_last/2 is nondet.      % "... can be used to generate ..."
...
:- map_queue(det, ?) is det. % This is the expected mode, a determinate producer.
:- map_queue(semidet, ?) is semidet.
:- map_queue(multi, ?) is multi.
:- map_queue(failing, ?) is semidet. % (!) The Pred will not be called if the queue is empty, so success is always possible.
:- map_queue(nondet, ?) is nondet.
...
:- map_queue(semidet, ?, ?) is semidet.
:- map_queue(multi, ?, ?) is multi.
:- map_queue(failing, ?, ?) is semidet. % (!) The Pred will not be called if the queues are empty, so success is always possible.
:- map_queue(nondet, ?, ?) is nondet.
...
:- map_queue_list(det, ?, ?) is det. % This is the expected mode, a determinate producer.
:- map_queue_list(semidet, ?, ?) is semidet.
:- map_queue_list(multi, ?, ?) is multi.
:- map_queue_list(failing, ?, ?) is semidet. % (!) The Pred will not be called on empty input.
:- map_queue_list(nondet, ?, ?) is nondet.
...
:- map_list_queue(det, ?, ?) is det. % This is the expected mode, a determinate producer.
:- map_list_queue(semidet, ?, ?) is semidet.
:- map_list_queue(multi, ?, ?) is multi.
:- map_list_queue(failing, ?, ?) is semidet. % (!) The Pred will not be called on empty input.
:- map_list_queue(nondet, ?, ?) is nondet.
...
:- some_queue/2 is nondet.      % expected to backtrack regardless of the determism of the Pred.
...
:- some_queue/3 is nondet.      % expected to backtrack regardless of the determism of the Pred.
