...
:- jasper_object_class_name/3 is det.
jasper_object_class_name(JVM, Object, Class) :-
...
:- jasper_initialize/1 is det.
jasper_initialize(JVM) :-
	jasperi_initialize(0,'',[],JVM).

%%% jasper_deinitialize(+JVM)
%%% Throws and error if the JVM could not be deinitialized.
:- jasper_deinitialize/1 is det.
jasper_deinitialize(JVM) :-
...
:- jasper_create_global_ref/3 is det.
jasper_create_global_ref(JVM,Ref,GlobalRef) :-
...
:- jasper_create_local_ref/3 is det.
jasper_create_local_ref(JVM,Ref,LocalRef) :-
...
:- jasper_delete_global_ref/2 is det.
jasper_delete_global_ref(JVM,GlobalRef) :-
...
:- jasper_delete_local_ref/2 is det.
jasper_delete_local_ref(JVM,LocalRef) :-
...
%%% jasper_is_jvm(+JVM).
:- jasper_is_jvm/1 is semidet.
jasper_is_jvm(JVM) :-
   jasper_jvmref(JVM).


%%% jasper_is_object(+Object).
:- jasper_is_object/1 is semidet.
jasper_is_object(Object) :-
   jasper_objref(dummy,Object).

%%% jasper_is_object(+JVM,+Object).
:- jasper_is_object/2 is semidet.
jasper_is_object(JVM,Object) :-
   jasper_objref(JVM,Object).

%% jasper_is_null(+JVM, +Object)
:- jasper_is_null/2 is semidet.
jasper_is_null(JVM, Object) :-
...
:- jasper_is_same_object/3 is semidet.
...
:- jasper_is_instance_of/3 is semidet.
jasper_is_instance_of(JVM,Object,ClassName) :-
...
jasper_convert_primitive_type(0'V, void).
