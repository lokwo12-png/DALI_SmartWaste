...
#define dvar_init_ix(m_dvar,m_attr,m_var)		\
{							\
  TAGGED m_tmp = (m_attr);				\
  TAGGED m_min, m_max, m_set;				\
  (m_dvar)->attr_ref = (SP_globref)m_tmp;		\
  (m_dvar)->var_ref = (SP_globref)(m_var);		\
  DomFromAttr(m_tmp,m_tmp); /* get dom/4 term */	\
  m_min = DomainMin(m_tmp);				\
  m_max = DomainMax(m_tmp);				\
  m_set = DomainSet(m_tmp);				\
  (m_dvar)->min = m_min;				\
  (m_dvar)->max = m_max;				\
  (m_dvar)->set = m_set;				\
  (m_dvar)->flags = DV_SET_OK |				\
    (m_min==m_max ? DV_PRUNED_VAL : 0) |		\
    ((CTagToCdr(m_set)==EmptySet) ? DV_INTERVAL : 0);	\
}
...
/* Purpose:
   ...
   Synopsis:
     ...
     SP_integer   dvar_interval_count(wam,dvar);
     SP_integer   dvar_value_count(wam,dvar);
   Arguments:
     Dvar dvar          - the dvar
*/
...
/* Purpose:
   ...
   Synopsis:
     ...
     SP_BOOL dvar_intersect_set(dvar, set);
     SP_BOOL dvar_intersect_interval_l(dvar, min, max);
     SP_BOOL dvar_intersect_interval_t(dvar, tmin, tmax);
...
#define dvar_compare_interval_t(DVAR,MIN,MAX) \
  dvar_compare_interval_t_(DVAR,MIN,MAX)

extern int
dvar_compare_interval_t_(Wam wam, Dvar dvar, TAGGED tmin, TAGGED tmax);

#define dvar_intersect_set(DVAR,S) \
  dvar_intersect_set_(wam, DVAR,S)

extern int
dvar_intersect_set_(Wam wam, Dvar dvar, TAGGED set);

#define dvar_intersect_interval_l(DVAR,MIN,MAX) \
  dvar_intersect_interval_t_(wam, DVAR,MakeSmall0(MIN),MakeSmall0(MAX))

#define dvar_intersect_interval_t(DVAR,MIN,MAX) \
  dvar_intersect_interval_t_(wam, DVAR,MIN,MAX)

extern int
dvar_intersect_interval_t_(Wam wam, Dvar dvar, TAGGED tmin, TAGGED tmax);
...
extern SP_BOOL
dvar_fix_interval_export_ix(Wam wam, Dvar dvar, TAGGED lbt, TAGGED ubt);

extern SP_BOOL
dvar_fix_value_export_ix(Wam wam, Dvar dvar, TAGGED value);
...
typedef struct bvar *Bvar;

#define bvar_is_integer(bvar) ((bvar)->min==(bvar)->max)
#define bvar_min_l(bvar) GetSmall0((bvar)->min)
#define bvar_max_l(bvar) GetSmall0((bvar)->max)
#define bvar_min_int(bvar) GetSmall_int0((bvar)->min)

#define bvar_refresh(bvar) {				\
  TAGGED m_temp = RefGlob((bvar)->attr_ref);		\
							\
  DomFromAttr(m_temp,m_temp);			\
  (bvar)->min = DomainMin(m_temp);			\
  (bvar)->max = DomainMax(m_temp);			\
}							\

/* Purpose: 
     (Re)Initialize bvar when entering filtering alg.
   Synopsis:
     void bvar_init(bvar, attr_ref, var_ref);
   Arguments:
     Bvar bvar            - the bvar
     SP_globref attr_ref - SP_globref holding Prolog FD attribute
     SP_globref var_ref  - SP_globref holding Prolog FD varable
...
/* accellerator for bvar_fix_value + bvar_export in one go.  Can GC. */
extern SP_BOOL
