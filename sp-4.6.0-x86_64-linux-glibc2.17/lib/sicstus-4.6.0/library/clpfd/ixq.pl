...
:- attribute fd_attribute/4.
%% index, domain mutable, suspensions mutable.

fd_purge(X) :-
	var(X),
	get_atts(X, fd_attribute(_,_,_,_)), !,
	'$fd_purge'(X).
fd_purge(_).
...
purify_var(X) :-
	var(X),
	put_atts(X, -fd_attribute(_,_,_,_)).
...
get_fd_suspensions(X, Lists) :-
	get_atts(X, fd_attribute(_,_,ListsM,_)),
	get_mutable(Lists, ListsM).

get_fd_aliases(X, Aliases) :-
	get_atts(X, fd_attribute(_,_,_,AliasesM)),
