			 ...
			 TAGGED min, TAGGED max, TAGGED size, SP_BOOL is_fd)
{
  ...
  case CONST_TAG:
    ...
    if (TagIsATM(argument)) {
      ...
      *h++ = TaggedZero;
      *h++ = TaggedZero;
      /* 6*/*h++ = functor_Dmutable;
      *h++ = attr + WD(9);
      *h++ = TaggedZero;
      /* 9*/*h++ = functor_dom4;
      *h++ = MakeList(w->global_top+14);
      /*11*/*h++ = argument;
      *h++ = argument;
      *h++ = TaggedOne;
      /*14*/*h++ = MakeList(w->global_top+11);
      *h++ = atom_nil;
      w->global_top = h;
    }
    return attr;
	     ...
	     TAGGED trigger)
{
  DEREF(var,var);
  if (IsVar(var))
    fd_link_attr(wam, fd_check_argument(wam,var,Inf,Sup,Sup,TRUE), key, item, trigger);
}
...
int fd_dequeue(Wam wam, TAGGED *item)
{
 ...
 start:
  ...
  case FD_QUEUE_IFF:
	...
	q->head = s;
	if (s == q->tail)
	  cur->hint &= ~(1U << FD_QUEUE_IFF);
	Deref1(entvar,CTagToArg(term,4));
	if (IsVar(entvar)) {
	  BindHVA(entvar,key);
	}
	if (key==value) {
	  term = CTagToArg(term,1);
	  Deref1(entvar,CTagToArg(term,4));
	  if (IsVar(entvar)) {
	    *item = term;
	    return 1;
...
static void fd_enqueue_list_gc_ix(Wam wam, struct propagator *cur, int index, TAGGED safe_time_stamp)
{
  ...
  if (TagIsLST(list))
    fd_enqueue_append_nomut(wam, list, FD_QUEUE_IX);
}
...
static void fd_enqueue_list_ix(Wam wam, struct propagator *cur, int index, TAGGED safe_time_stamp)
{
  ...
  (void)safe_time_stamp;
  if (TagIsLST(list))
    fd_enqueue_prepend_nomut(wam, list, FD_QUEUE_IX);
}
...
prolog_fd_purge(Wam wam, SP_term_ref Var)
{
  ...
  // entail_iff(wam, CTagToArg(susps,FD_LIST_VAL_IFF)); /* assume that relevant indexicals will be entailed anyway */
  ...
  *tail = aliases;
  ...
  delete_attributes(var, fd.fd_module, 0);
}
