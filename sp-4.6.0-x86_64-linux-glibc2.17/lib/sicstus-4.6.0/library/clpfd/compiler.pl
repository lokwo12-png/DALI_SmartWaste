...
set_expression_check(D1\/D2, Set, _, _) :-
	'$fd_set_expression'(D1\/D2, Set), !.
set_expression_check(D1\/D2, Set, Goal, ArgNo) :- !, % avoid deep recursion for long chains of these
	set_or_flatten(D1\/D2, Flat, []),
	(   foreach(X,Flat),
	    foreach(Y,Ys),
	    param(Goal,ArgNo)
	do  set_expression_check(X, Y, Goal, ArgNo)
	),
	fdset_union(Ys, Set).
set_expression_check(D1\/D2, Set, Goal, ArgNo) :- !,
	set_expression_check(D1, Set1, Goal, ArgNo),
...
t_rel_c(#<, T, Con) -->
	({fd_dvar(T), fd_integer(Con)} -> {Con1 is Con-1}, ['t=<c'(T,Con1)]; ['x<y'(T,Con)]).
t_rel_c(#\=, T, Con) -->
	({fd_dvar(T), fd_integer(Con)} -> ['t\\=c'(T,Con)]; ['x\\=y'(T,Con)]).
t_rel_c(#>=, T, Con) -->
	({fd_dvar(T), fd_integer(Con)} -> ['t>=c'(T,Con)]; ['x=<y'(Con,T)]).
t_rel_c(#>, T, Con) -->
	({fd_dvar(T), fd_integer(Con)} -> {Con1 is Con+1}, ['t>=c'(T,Con1)]; ['x<y'(Con,T)]).

t_rel_u_c(#=, T, U, Con) --> ['t=u+c'(T,U,Con)].
t_rel_u_c(#=<, T, U, Con) --> ['t=<u+c'(T,U,Con)].
t_rel_u_c(#<, T, U, Con) --> {Con1 is Con-1}, ['t=<u+c'(T,U,Con1)].
t_rel_u_c(#\=, T, U, Con) --> ['t\\=u+c'(T,U,Con)].
t_rel_u_c(#>=, T, U, Con) --> ['t>=u+c'(T,U,Con)].
t_rel_u_c(#>, T, U, Con) --> {Con1 is Con+1}, ['t>=u+c'(T,U,Con1)].
...
t_u_rel_c(#=<, T, U, Con) --> ['t+u=<c'(T,U,Con)].
t_u_rel_c(#<, T, U, Con) --> {Con1 is Con-1}, ['t+u=<c'(T,U,Con1)].
t_u_rel_c(#\=, T, U, Con) --> ['t+u\\=c'(T,U,Con)].
t_u_rel_c(#>=, T, U, Con) --> ['t+u>=c'(T,U,Con)].
t_u_rel_c(#>, T, U, Con) --> {Con1 is Con+1}, ['t+u>=c'(T,U,Con1)].
...
reify(scalar_product(As,Xs,Rel,S), B) -->
	[scalar_product_reif(As,Xs,Rel,S,B)].
...
:- remove_from_conjunction/3 is nondet.
