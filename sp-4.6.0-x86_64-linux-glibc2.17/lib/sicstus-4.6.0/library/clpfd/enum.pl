...
can_fast_labeling(bb(MinMax,_,Ref,_,33554431), param(Sel,Enum,off), Encoding, param(Obj,Ref)) :-
	(   MinMax = minimize(Obj) -> Bit = 0x10000
	;   MinMax = maximize(Obj) -> Bit = 0x20000
	),
	encode_variable_choice(Sel, EncSel),
	Enum =.. [Type,Pivot],
	encode_branch_type(Type, EncType),
	encode_branch_pivot(Pivot, EncPivot),
	Encoding is EncSel + EncType + EncPivot + Bit.
...
encode_variable_choice(leftmost,         0x00001).
encode_variable_choice(input_order,      0x00001).
encode_variable_choice(min,              0x00002).
encode_variable_choice(smallest,         0x00002).
encode_variable_choice(max,              0x00004).
encode_variable_choice(largest,          0x00004).
encode_variable_choice(ff,               0x00008).
encode_variable_choice(first_fail,       0x00008).
encode_variable_choice(ffc,              0x00010).
encode_variable_choice(most_constrained, 0x00010).
encode_variable_choice(anti_first_fail,  0x00020).
encode_variable_choice(occurrence,       0x00040).
encode_variable_choice(max_regret,       0x00080).

encode_branch_type(enum,   0x00100).
encode_branch_type(step,   0x00200).
encode_branch_type(bisect, 0x00400).

encode_branch_pivot(up,     0x01000).
encode_branch_pivot(down,   0x02000).
encode_branch_pivot(median, 0x04000).
encode_branch_pivot(middle, 0x08000).
...
labeling_cont(enum(Arg), X, L, _LL, L, BB, BB1) :-
	get_atts(X, fd_attribute(_,DomM,_,_)),
