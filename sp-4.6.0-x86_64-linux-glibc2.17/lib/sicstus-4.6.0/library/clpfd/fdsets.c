...
SP_integer fd_middle(TAGGED set) {
  TAGGED range;
...
SP_integer fd_median(TAGGED set) {
  SP_integer median;
  SP_integer count = (fd_size(set)-1)/2;
...
static TAGGED *fd_neg_internal(Wam wam, TAGGED d1, TAGGED toffset, TAGGED *valuep)
{
  if (d1!=EmptySet) {
    TAGGED *h, r1, b1, e1;
...
SP_BOOL fd_intersect(TAGGED d1, TAGGED d2)
{
  TAGGED r1, b1, e1, r2, b2, e2;
...
list_and_head_tail:
  ...
  b1 = RangeMin(r1);   e1 = RangeMax(r1);
head_tail_and_head_tail:
  if (FDlt(e1,b2)) {		/* BEFORE */
    goto list_and_head_tail;
  } else if (FDgt(b1,e2)) {	/* AFTER */
    goto head_tail_and_list;
  } else {
    return TRUE;
  }
...
SP_BOOL fd_intersect_interval(TAGGED d1, TAGGED b2, TAGGED e2)
{
  TAGGED r1, b1, e1;
...
SP_BOOL fd_intersect_intervals(TAGGED b1, TAGGED e1, TAGGED b2, TAGGED e2)
{
  if (FDlt(e1,b2) || FDgt(b1,e2)) {	/* BEFORE || AFTER */
    return FALSE;
  } else {
    return TRUE;
  }
...
static TAGGED *insert_subterm(Wam wam, TAGGED *p, TAGGED term)
{
  TAGGED a, b;
  
  if (!TagIsSTR(term))
    return NULL;
  else if (TagToHeadfunctor(term)==fd.functor_interval2) {
    a = CTagToArg(term,1);
    b = CTagToArg(term,2);
    DerefSwitch(a,return NULL;);
    DerefSwitch(b,return NULL;);
  } else if (TagToHeadfunctor(term)==fd.functor_set1) {
    a = CTagToArg(term,1);
    DerefSwitch(a,return NULL;);
    b = a;
  } else {
  ...
  if ((!IsSmall(a) && a!=Inf) || (!IsSmall(b) && b!=Sup) || FDgt(a,b))
    return NULL;
  if (p < w->stack_start)
    if (p-4 < w->global_top || !AreSmall(b,p[2]) || Tge(b+IStep(1),p[2]))
      return NULL;
  p -= 4;
  p[2] = a;
  p[3] = b;
...
/* $fd_set_expression(+Range, -Set) */
void SPCDECL
prolog_fd_set_expression(Wam wam, 
			 SP_term_ref RangeR,
			 SP_term_ref SetR)
{
  TAGGED *p = w->stack_start;
  TAGGED *q = w->global_top;
  TAGGED expr = RefTerm(RangeR);
  TAGGED term;

  DerefSwitch(expr,goto fail;);
  while (TagIsSTR(expr) && TagToHeadfunctor(expr)==fd.functor_or2) {
    term = CTagToArg(expr,2);
    expr = CTagToArg(expr,1);
    DerefSwitch(term,goto fail;);
    DerefSwitch(expr,goto fail;);
    p = insert_subterm(wam, p, term);
    if (!p)
      goto fail;
  }
  p = insert_subterm(wam, p, expr);
  if (!p)
    goto fail;
  while (p < w->stack_start) {
    q[0] = MakeList(q+2);
    q[1] = MakeList(q+4);
    q[2] = p[2];
    q[3] = p[3];
  ...
  q[-3] = atom_nil;
  RefTerm(SetR) = MakeList(w->global_top);
  w->global_top = q;
  return;
 fail:
  SP_fail();
