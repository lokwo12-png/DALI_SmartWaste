		       ...
		       SP_term_ref ZeroOneAttr)
{
  ...
  count *= (19+FD_LINK_NEED); /* ix(9) + $mutable(3) + iff(5) + unit list(2) */
  FdMemRequireHeap(count,7);	
  ...
  for (type=mintype; type<=maxtype; type++) {
	...
	*q++ = ix;
	...
	fd_link(wam, X(2), fd.functor_val, MakeStructure(h), 0);
      } else {			/* post indexical, and link if not idempotent */
	h = w->global_top;
	h[0] = item;
	h[1] = atom_nil;
	w->global_top = h+2;
	fd_enqueue_append_nomut(wam, MakeList(h), FD_QUEUE_IX);
      }
    }
  }
  SP_ASSERT(w->global_top <= w->heap_warn);
		      ...
		      SP_term_ref Global)
{
  ...
  FdMemRequireHeap(nvars*FD_LINK_NEED+12, 0);
  h = w->global_top;
  h[0] = functor_Dmutable;
  h[1] = RefTerm(State);
  h[2] = TaggedZero;
