...
:- samsort/3 is det.            % Never nondet, even if Order is nondet.
...
:- merge/4 is det.              % Never nondet, even if the Order is nondet.
...
%@  lists.
%@  @c [PM] 4.5.1 not true for a long time:
%@  @c Note that it will not work properly when @var{Merged} is already
%@  @c instantiated.
:- keymerge/3 is det.
