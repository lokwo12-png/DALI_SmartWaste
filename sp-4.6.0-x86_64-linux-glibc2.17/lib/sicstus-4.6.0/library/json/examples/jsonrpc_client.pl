...
/*
  ...
  bash$ /usr/local/sicstus4.5.0/bin/sicstus --nologo --noinfo --goal 'jsonrpc_client_main,halt.' -l /usr/local/sicstus4.5.0/lib/sicstus-4.5.0/library/json/examples/jsonrpc_client.pl
  state ==> State is @(null)
  state:=4 ==> State was @(null)
  state ==> State is 4
  once 'Result is StateIn+1, StateOut=Result.' ==> State is 5
  once 'Result is StateIn+1, StateOut=Result.' ==> State is 6
  once 'Result is StateIn+Increment,StateOut=Result.' ==> State is 11
  call 'member(E,[10,20,30]), Inc is Multiplier*E, Result is StateIn+Inc.' ==> Result is 111
  retry ==> (next) Result is 211
  cut ==> Result is @(null)
  call 'member(E,[10,20,30]), Inc is Multiplier*E, Result is StateIn+Inc.' ==> Result is 111
  retry ==> (next) Result is 211
  retry ==> (next) Result is 311
  retry ==> failed (this is expected)
  once 'foo is bar.' ==> Exception: Exception 'type_error(evaluable,bar/0)' (this is expected)
  once ==> Result is 'Bye'
  bash$
*/
...
jsonrpc_client_main :-
        ...
        (   Argv = [_,PlPath|_] ->
            true
        ;   PlPath = '$SP_LIBRARY_DIR/json/examples/jsonrpc_server'
        ),
...
interact(ProcessIn, ProcessOut) :-
        ...
        (   Goal1 = 'Result is StateIn+1, StateOut=Result.',
            ...
            (   Parsed4 = success(Result4) ->
                format(user_output, 'State is ~q~n', [Result4])
            ;   format(user_error, '! UNEXPECTED ~q~n', [Parsed4])
            )
        ),
        ...
        (   Goal2 = 'Result is StateIn+1, StateOut=Result.',
            ...
            (   Parsed5 = success(Result5) ->
                format(user_output, 'State is ~q~n', [Result5])
        ...
        (   format(user_output, 'retry ==> ', []),
            ...
            (   Parsed8 = success(Result8) ->
                format(user_output, '(next) Result is ~q~n', [Result8])
        ...
        (   format(user_output, 'cut ==> ', []),
            ...
            (   Parsed9 = success(Result9) ->
                format(user_output, 'Result is ~q~n', [Result9])
        ...
        (   format(user_output, 'retry ==> ', []),
            ...
            (   Parsed11 = success(Result11) ->
                format(user_output, '(next) Result is ~q~n', [Result11])
        ...
        (   format(user_output, 'retry ==> ', []),
            ...
            (   Parsed12 = success(Result12) ->
                format(user_output, '(next) Result is ~q~n', [Result12])
        ...
        (   format(user_output, 'retry ==> ', []),
            ...
            (   Parsed13 = success(Result13) ->
                format(user_output, '(next) Result is ~q~n', [Result13])
            ;   Parsed13 = failed ->
                format(user_output, 'failed (this is expected)~n', [])
            ;   otherwise ->
