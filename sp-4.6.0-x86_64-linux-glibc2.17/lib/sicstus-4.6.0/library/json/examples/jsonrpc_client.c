...
/*-
  ...
  /tmp/json_test
  bash$ ls
  bash$ cc /usr/local/sicstus4.5.0/lib/sicstus-4.5.0/library/json/examples/jsonrpc_client.c -o jsonrpc_client
  bash$ ls
  jsonrpc_client
  bash$ ./jsonrpc_client /usr/local/sicstus4.5.0/bin/sicstus
  state ==> State is null
  state:=4 ==> State was null
  state ==> State is 4
  once(Result is StateIn+1, StateOut=Result) ==> Result=5
  once(Result is StateIn+1, StateOut=Result) ==> Result=6
  Multiplier=10, call(member(E,[10,20,30]), Inc is Multiplier*E, Result is StateIn+Inc) ==> First Result=106
  retry ==> (next) Result=206
  cut ==> (next) Result=null
  Multiplier=10, call(member(E,[10,20,30]), Inc is Multiplier*E, Result is StateIn+Inc) ==> First Result=106
  retry ==> (next) Result=206
  retry ==> (next) Result=306
  retry ==> Prolog failed (this is expected)
  once(foo is bar) ==> Prolog threw an exception (this is expected)
  quit ==> (next) Result="Bye"
  bash$

  Windows transcript:

  FIXME: Windows Instructions.
   . ./env.sh && cl /MD /Zi /RTCs /RTCc /RTCu /GS /MD /WX /W3  /Gr -D_UNICODE -DLOGGING=1 library/json/examples/jsonrpc_client.c
*/
...
#endif  /* USE_POSIX_API */
{
  ...
  TCHAR *pl_path = (argc > 2 ? argv[2] : TEXT("$SP_LIBRARY_DIR/json/examples/jsonrpc_server"));
