...
%@
%@  @strong{Please note}: You should probably not use this module.
%@  The ordered representation used in @code{library(ordsets)} is @strong{much more
...
:- set_order/3 is semidet.      % (fails if incomparable)
set_order(Xs, Ys, R) :-
...
%@  @end group
%@  @end example
:- disjoint_union/3 is semidet. % Because disjoint(Set1, Set2) is not just a precondition, it is actively tested.
disjoint_union(Set1, Set2, Union) :-
	(   foreach(X,Set1),
	    fromto(Union,[X|S],S,Set2),
	    param(Set2)
