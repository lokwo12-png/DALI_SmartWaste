...
:- checkbag(det, ?) is det.    % This is the expected mode, a determinate producer.
:- checkbag(semidet, ?) is semidet.
:- checkbag(multi, ?) is multi.
:- checkbag(failing, ?) is semidet. % (!) Pred will not be called on empty input.
:- checkbag(nondet, ?) is nondet.
...
:- mapbag(det, ?) is det.    % This is the expected mode, a determinate producer.
:- mapbag(semidet, ?) is semidet.
:- mapbag(multi, ?) is multi.
:- mapbag(failing, ?) is semidet. % (!) Pred will not be called on empty input.
:- mapbag(nondet, ?) is nondet.
...
:- somebag/2 is nondet. % Always nondet, regardless of Pred.
