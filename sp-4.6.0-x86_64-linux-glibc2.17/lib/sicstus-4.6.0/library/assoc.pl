...
:- map_assoc(det, ?) is det.  % This is the expected mode, a determinate producer.
:- map_assoc(semidet, ?) is semidet.
:- map_assoc(multi, ?) is multi.
:- map_assoc(failing, ?) is semidet. % (!) Pred will not be called on empty input.
:- map_assoc(nondet, ?) is nondet.
