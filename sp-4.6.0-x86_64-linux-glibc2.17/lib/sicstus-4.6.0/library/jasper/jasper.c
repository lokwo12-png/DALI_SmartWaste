...
#define DELETE_LOCAL_REF(ENV, REF) do{fprintf(stderr, "NOT deleting local ref %" SPRIdINTEGER "\n", (SP_integer)(REF));fflush(stderr);}while(0)
#define DELETE_GLOBAL_REF(ENV, REF) do{fprintf(stderr, "NOT deleting global ref %" SPRIdINTEGER "\n", (SP_integer)(REF));fflush(stderr);}while(0)
...
#if JASPER_DBG && !DISABLE_DBG_PRINT_OBJECT_CLASS
...
{
#if JASPER_DBG+0 > 0
  fprintf(stderr, "SPJavaGlueErrorM: arg# %d, `%s', ", argnum, ((err_msg != NULL) ? err_msg : "<<NULL>>"));fflush(stderr);
#endif
  (void)argnum;
...
#if JASPER_DBG
      fprintf(stderr, "SPJavaGluePropagateJavaExceptionM: arg# %d, `%s' (this is expected), ", argnum, err_msg);fflush(stderr);
      (*jnienv)->ExceptionDescribe(jnienv); /* questionable */
      fprintf(stderr, "\n");fflush(stderr);
      /* abort(); */
#endif
      /* [PM] 4.4.2 Must clear the exception _before_ calling NewGlobalRef(). */
      (*jnienv)->ExceptionClear(jnienv);
...
#if JASPER_DBG
  fprintf(stderr, "SPJavaGlueSPTermM %s term=%d \n", (init ? "init from" : "ignored"), term);fflush(stderr);
#endif
...
static jboolean jasper_DescribeClass(JNIEnv *jnienv, jobject jobj)
{
  ...
  jname = (jstring)(*jnienv)->CallObjectMethod(jnienv, clazz, mid); if (!jname) goto barf5;
            ASSERT_NO_EXCP(jnienv,__FUNCTION__); /* Do not expect exceptions here. */
...
static int SPJavaGlueGetAtomM(SPAPI_ARG_PROTO_DECL JNIEnv *jnienv, jobject obj /* SPCanonicalAtom or SPTerm */, SP_atom *p_atom, int argnum, const char *err_msg, jboolean threadservermode)
{
  ...
  if (threadservermode) {     /* [PD] 3.9.2 Only java/lang/String allowed */
    ...
    string = obj; /* [PM] 4.4.2 for freeing the codes(!) */
 ...
 cleanup:
  ...
  SP_ASSERT(codes != NULL ? string != NULL : 1);
...
#if JASPER_DBG
  fprintf(stderr, "SPJavaGluePostPutCodesM \"%s\"\n", (codes ? codes : "<<ERROR: codes==NULL!! >>"));fflush(stderr);
#endif
  JASPER_DBGLINE(SPJavaGluePostPutCodesM);
  if((!codes)
...
#define SET_JNIENV_EXTRA(JNIENV) do{ \
   fprintf(stderr, "Ensuring extra capacity (%d)\n", (int)(*(JNIENV))->EnsureLocalCapacity((JNIENV), 42)); fflush(stderr); \
...
/*
  As of Java 11, these are the JNI version used with various Java versions:
...
https://docs.oracle.com/javase/10/docs/specs/jni/functions.html#version-information
*/

/* From the Java 10 JNI Specification. */
#define SP_JNI_VERSION_1_1 0x00010001
#define SP_JNI_VERSION_1_2 0x00010002
#define SP_JNI_VERSION_1_4 0x00010004
#define SP_JNI_VERSION_1_6 0x00010006
#define SP_JNI_VERSION_1_8 0x00010008
#define SP_JNI_VERSION_9   0x00090000
#define SP_JNI_VERSION_10  0x000a0000

/* This is the lowest version we can use (for JNI_OnLoad and attach to
   pre-existing thread).

   If you change this you need to change the error message for
   JNI_EVERSION

   [PM] 4.4.2 Ripped out all the configurability. We now require a fixed JNI-version (1.6).
*/
#define SP_JNI_VERSION_NEEDED JNI_VERSION_1_6 /* Here only: Use the jni.h name, not SP_JNI_VERSION_XXX. */
...
static int jasper_int_handle_exception(SPAPI_ARG_PROTO_DECL JNIEnv *env, char *desc, int error)
{
  ...
  if (have_jexcp) {
#if JASPER_DBG /* [PM] 4.4.2 No longer unconditionally write it in release builds. */
    jasper_DescribeException(env); 
#endif


    /* Never leave lingering Java exceptions. */
...
void SPCDECL deinit_jasper(SPAPI_ARG_PROTO_DECL int when)
{
  DbgBreakPoint();
#if JASPER_DBG>1
  fprintf(stderr, "\ndeinit_jasper(%d)\n", when); fflush(stderr);fflush(stderr);
#endif
  (void)when;
#if MULTI_SP_AWARE
...
#if SP_JNI_VERSION_NEEDED == SP_JNI_VERSION_1_6
        "1.6"
#else
#error "[PM] 4.4.2 The version string must be updated to match SP_JNI_VERSION_NEEDED"
#endif
        " or higher";
      goto cleanup;
    }
  ...
  if (length >= size)
    {
      ...
      size = length+1;
#if JASPER_DBG>42
      fprintf(stderr, "SP_realloc(\"%s\" (%" SPRIdSZ "), %" SPRIdSZ ")\n", buf, (size_t)strlen(buf), (size_t) size);fflush(stderr);
#endif
...
static SP_term_ref jasperi_initialize(SPAPI_ARG_PROTO_DECL int numopts, SP_term_ref classpath, SP_term_ref tr)
{
...
#if JASPER_DBG || JASPER_CHECK_JNI
    "-Xcheck:jni",
    "-verbose:jni",
#endif  /* JASPER_CHECK_JNI */
    

#if JASPER_DBG
    "-DSICStus.debugLevelDefault=" STRINGISIZE(JASPER_DBG),
#endif /* JASPER_DBG */
  ...
  if (STATE_VARIABLE(jvm) != NULL)
    ...
    { /* No JVM available. Lets create one. */
      ...
      {
        vm_args.version = SP_JNI_VERSION_NEEDED; /* Always */
      }

      {
        vm_args.ignoreUnrecognized = JNI_TRUE;  /* Always */
      }
...
#if !FORCE_BUILD && SICSTUS_VERSION > 40600 /* [PM] 4.3.2 postpone */
#error "This function and several others leaks SP_malloced memory on error"
        ...
        for (SP_put_term(tail,tr),i = NUM_EXTRA_OPTS; SP_get_list(tail,elem,tail) && (i-NUM_EXTRA_OPTS)<numopts; i++)
          {
            ...
            if (strncmp(optionString_ienc, "JNI_VERSION_", strlen("JNI_VERSION_")) == 0)
              {
                /* [PM] 4.4.2 Ignore JNI_VERSION_*, for backward compatibility. */
      ...
      for (i = 0; i < numopts; i++)
        {
          fprintf(stderr, "option-string[%d] = %s\n",i,options[i].optionString);fflush(stderr);
        }
#endif /* JASPER_DBG */


      /* Call a hook that turns off some signal handlers a.k.a -Xrs. */
      {
...
#if JASPER_DBG
      {
        ...
        switch (version) {
        case SP_JNI_VERSION_1_2: version_string = "JNI_VERSION_1_2"; break;
        case SP_JNI_VERSION_1_4: version_string = "JNI_VERSION_1_4"; break;
        case SP_JNI_VERSION_1_6: version_string = "JNI_VERSION_1_6"; break;
        case SP_JNI_VERSION_1_8: version_string = "JNI_VERSION_1_8"; break;
        case SP_JNI_VERSION_9: version_string = "JNI_VERSION_9"; break;
        case SP_JNI_VERSION_10: version_string = "JNI_VERSION_10"; break;
        default: SP_ASSERT(0); version_string = "<<UNKNOWN>>";
        }
  ...
  */
  if (!jvm_already_present)
    {
#if JASPER_DBG > 1
      fprintf(stderr, "*** Creating SICStus object (JNI %p, JVM %p)\n", (void*)STATE_VARIABLE(state_jnienv), (void*)STATE_VARIABLE(jvm));fflush(stderr);
#endif
...
static void dbg_print_object_class(JNIEnv *jnienv, jobject jobj, char *extra_info)
{
  jclass clazz = NULL;          /* needs DeleteLocalRef() */
  jclass clazzclazz = NULL;     /* needs DeleteLocalRef() */
  jobject jname = NULL;         /* needs DeleteLocalRef() */
  int const needed_capacity = 3;
  jmethodID mid;

  ASSERT_NO_EXCP(jnienv, "dbg_print_object_class 1");

#if JASPER_DBG
  fprintf(stderr, "dbg_print_object_class of object %" SPRIdINTEGER "\n", (SP_integer) jobj);fflush(stderr);
#endif
  if (jobj == NULL) goto cleanup;

  if ((*jnienv)->EnsureLocalCapacity(jnienv, needed_capacity) != JNI_OK) {
    fprintf(stderr, "dbg_print_object_class EnsureLocalCapacity(%d) failed\n", (int) needed_capacity); fflush(stderr);
    goto barf;
  }
  ...
  if ((*jnienv)->ExceptionCheck(jnienv)) jasper_DescribeException(jnienv);
#endif
  clazzclazz = (*jnienv)->GetObjectClass(jnienv,clazz);
  ASSERT_NO_EXCP(jnienv, "dbg_print_object_class 3");
  mid = (*jnienv)->GetMethodID(jnienv,clazzclazz,"getName","()Ljava/lang/String;");
  ...
  if (mid)
    {
      ...
      if ((*jnienv)->ExceptionCheck(jnienv))
        {
          JASPER_DBGLINE(dbg_print_object_class);
          jasper_DescribeException(jnienv);
        }
      else
        {
 ...
 cleanup:
  if (jname != NULL) {
    (*jnienv)->DeleteLocalRef(jnienv, jname);
    jname = NULL;
  }
  if (clazz != NULL) {
  ...
  if (clazzclazz != NULL) {
    (*jnienv)->DeleteLocalRef(jnienv, clazzclazz);
    clazzclazz = NULL;
  }
  return;
 barf:
...
#if JASPER_DBG
  fprintf(stderr, "jasperi_process_meta_args: %s method mode=%s\n", (instance ? "instance" : "static"), (calling ? "calling" : (returning ? "returning" : (cleanup ? "cleanup" : "ERROR"))));fflush(stderr);
#endif
  ...
  for (argno = first_argno, i = 1; argno <= numargs; argno++)
    {
...
#if JASPER_DBG
          fprintf(stderr, "jasperi_process_meta_args: plus_or_minus=`%s'==%s\n", SP_string_from_atom(plus_or_minus), (inarg ? "input arg" : "output arg"));fflush(stderr);
#endif
        }
                  ...
                  if (returning) /* (java/lang/String) */
                    {
...
#if JASPER_DBG > 1
                      {
                        ...
                        fprintf(stderr, "[-string] = %s\n", ( SP_get_string(tmp_tr, &string) ? string : "<<ERROR: SP_get_string failed>>"));fflush(stderr);
                      }
#endif
                    }
                }
      ...
      if (returning && !inarg)               /* a.k.a returning && (retval || !inarg) */
        {
#if JASPER_DBG
          fprintf(stderr, "jasperi_process_meta_args: unifying return value (arg is %s (%d), tmp_arg is %s (%d)\n", (SP_is_variable(arg) ? "variable" : "NOT variable!"), SP_term_type(arg), (SP_is_atom(tmp_tr) ? "atom" : "not atom"), SP_term_type(tmp_tr));fflush(stderr);
#endif
...
#if JASPER_DBG
  fprintf(stderr, "make_arg_array: typesig==%s\n", typesig);fflush(stderr);
#endif

#if JASPER_PUSH_LOCAL_REFS
  /* [PM] 4.4.2 FIXME: Investigate: #error "[PM] 3.9.2 JASPER_PUSH_LOCAL_REFS should be off" */
...
#if JASPER_DBG
  fprintf(stderr, "*** jasperi_call_static1_C jnienv: %p\n", (void*)jnienv);fflush(stderr);
#endif /* JASPER_DBG */

#if LOCALREFS_RESET && !jasperi_call_static1_C_DISABLE
    if ((*jnienv)->PushLocalFrame(jnienv, 0+LOCALREFS_RESET_EXTRA) < 0) { return; }
#endif
            ...
            case 'V':
              ...
              #if JASPER_DBG>1
              {
                ...
                for (i = 1; i <= state.numargs; i++)
                  {
                    fprintf(stderr, "CallStaticVoidMethodA %s::%s jargs[%d]==%p (%s)\n", className, methodName, i, ( state.is_object[i] ? (jargs)[i].l : 0 ), ( state.is_object[i] ? "object": "primitive"));fflush(stderr);
                  }
              }
              #endif
...
#if LOCALREFS_RESET && !jasperi_call_static1_C_DISABLE
#error "[PM] 4.4.2 Can not pop frame here. It would invalidate any local ref returned for [-object] in jasperi_process_meta_args()"
  (*jnienv)->PopLocalFrame(jnienv, NULL);
#endif
}
...
#if JASPER_DBG
  fprintf(stderr, "*** jasperi_call_instance1_C Calling %s, signature %s.\n", methodName, typesig);fflush(stderr);
#endif

  SET_JNIENV(jnienv,jvmref, {goto cleanup;});

#if LOCALREFS_RESET && !jasperi_call_instance1_C_DISABLED /* looks OK */
    if ((*jnienv)->PushLocalFrame(jnienv, 0+LOCALREFS_RESET_EXTRA) < 0) { return; }
#endif
  ...
  ****/
...

  {
    int result_is_object = FALSE;
    {
      ...
      fprintf(stderr, "%s:%d About to call instance method\n", __FILE__, (int) __LINE__); fflush(stderr);
      switch (return_type_sig[0])
        {
        case 'V':
          if (threadservermode) {
...
#if LOCALREFS_RESET && !jasperi_call_instance1_C_DISABLED
#error "[PM] 4.4.2 Can not pop frame here. It would invalidate any local ref returned for [-object] in jasperi_process_meta_args()"
  (*jnienv)->PopLocalFrame(jnienv, NULL);
#endif
}
...
{ /* FIXME:  JASPER_PUSH_LOCAL_REFS (need to be extra careful since it returns a local ref) */
  int barf = 1;
  JNIEnv *jnienv;
  jobject jobj = NULL;
  SP_term_ref tr = SP_new_term_ref();
 ...
 cleanup:
  ...
  if (!barf)
    {
      jobject lobj = ( jobj ? (*jnienv)->NewLocalRef(jnienv,jobj) : NULL );
      ASSERT_NO_EXCP(jnienv, "jasperi_create_local_ref_C");
      DBG_PRINT_OBJECT_CLASS(jnienv, lobj, "jasperi_create_local_ref_C (local ref)");

      jobject_to_termref(SPAPI_ARG jnienv, lobj, tr);
    }
  else
    {
#if JASPER_DBG
...
{                               /* FIXME: JASPER_PUSH_LOCAL_REFS */
...
#if JASPER_DBG > 1
  {
    ...
    fprintf(stderr, "In Java: calling DeleteGlobalRef(%ld)\n", (long)jobj); fflush(stderr);
    switch ((refType = (*jnienv)->GetObjectRefType(jnienv, jobj))) {
    case JNIInvalidRefType:
      fprintf(stderr, "In Java: calling DeleteGlobalRef(%ld), GetObjectRefType()==%s\n", (long)jobj, "JNIInvalidRefType"); fflush(stderr);
      break;
    case JNILocalRefType:
      fprintf(stderr, "In Java: calling DeleteGlobalRef(%ld), GetObjectRefType()==%s\n", (long)jobj, "JNILocalRefType"); fflush(stderr);
      break;
    case JNIGlobalRefType:
      fprintf(stderr, "In Java: calling DeleteGlobalRef(%ld), GetObjectRefType()==%s\n", (long)jobj, "JNIGlobalRefType"); fflush(stderr);
      break;
    case JNIWeakGlobalRefType:
      break;
      fprintf(stderr, "In Java: calling DeleteGlobalRef(%ld), GetObjectRefType()==%s\n", (long)jobj, "JNIGlobalRefType"); fflush(stderr);
    default:
      fprintf(stderr, "In Java: calling DeleteGlobalRef(%ld), GetObjectRefType()==%s (%d)\n", (long)jobj, "UNDEFINED", (int)refType ); fflush(stderr);
      break;
    }
  }
#endif  /* JASPER_DBG */
...
{                               /* FIXME: JASPER_PUSH_LOCAL_REFS */
  ...
  {
    jobjectRefType refType;
    fprintf(stderr, "In Java: calling DeleteLocalRef(%ld)\n", (long)jobj); fflush(stderr);
    switch ((refType = (*jnienv)->GetObjectRefType(jnienv, jobj))) {
    case JNIInvalidRefType:
      fprintf(stderr, "In Java: calling DeleteLocalRef(%ld), GetObjectRefType()==%s\n", (long)jobj, "JNIInvalidRefType"); fflush(stderr);
      break;
    case JNILocalRefType:
      fprintf(stderr, "In Java: calling DeleteLocalRef(%ld), GetObjectRefType()==%s\n", (long)jobj, "JNILocalRefType"); fflush(stderr);
      break;
    ...
    case JNIWeakGlobalRefType:
      ...
      fprintf(stderr, "In Java: calling DeleteLocalRef(%ld), GetObjectRefType()==%s\n", (long)jobj, "JNIGlobalRefType"); fflush(stderr);
    default:
      fprintf(stderr, "In Java: calling DeleteLocalRef(%ld), GetObjectRefType()==%s (%d)\n", (long)jobj, "UNDEFINED", (int)refType ); fflush(stderr);
      break;
    }
  }
#endif  /* JASPER_DBG */
...
{/* FIXME: JASPER_PUSH_LOCAL_REFS */
  ...
  {
          if ((*jnienv)->IsInstanceOf(jnienv, jobj, clazz))
          {
                  rc = 1;
          }
          else
