...
:- get_label/3 is semidet.      % index out of range may be intentional
...
:- map_tree(det, ?, ?) is det.    % This is the expected mode, a determinate producer.
:- map_tree(semidet, ?, ?) is semidet.
:- map_tree(multi, ?, ?) is multi.
:- map_tree(failing, ?, ?) is semidet. % (!) Pred will not be called on empty input.
:- map_tree(nondet, ?, ?) is nondet.
