...
%   The same as lists:nth1/3.
:- nth/3 is nondet.
nth(N, List, Element) :-
	nth1(N, List, Element).


%   The same as lists:nth1/4.
:- nth/4 is nondet.
...
%   The same as lists:max_member/2, but with different argument order.
:- max_list/2 is semidet.
max_list(ListOfNumbers, Max) :-
	max_member(Max, ListOfNumbers).

%   The same as lists:min_member/2, but with different argument order.
:- min_list/2 is semidet.
min_list(ListOfNumbers, Min) :-
	min_member(Min, ListOfNumbers).

%   The same as lists:sumlist/2.
:- sum_list/2 is det.
