...
:- db_open_env/2 is det.
db_open_env(EnvName, EnvRef) :-
        db_open_env(EnvName, 0, EnvRef).

:- db_open_env/3 is det.
:- db_open_env/3 is documented_as(db_open_env/2).
...
:- db_open/5 is documented_as(db_open/4).
:- db_open/5 is det.
...
:- db_erase/3 is documented_as(db_erase/2).
:- db_erase/3 is det.
...
:- db_compress/3 is documented_as(db_compress/2).
:- db_compress/3 is det.
...
:- db_make_iterator/3 is documented_as(db_make_iterator/2).
:- db_make_iterator/3 is det.
db_make_iterator(DBRef, Term, Iterator) :-
...
%@  @code{db_open/[4,5]}.
%@  
%@  @c In SICStus 3.12.0 @code{bdb:export/[2,3]} is available instead of
%@  @c @code{db_export/[2,3]}.
:- db_export/3 is det.
db_export(DBName, Options, ExportFile) :-
    export(DBName, Options, ExportFile).

:- db_export/2 is documented_as(db_export/3).
:- db_export/2 is det.
...
%@  @code{db_open/[4,5]}.
%@  
%@  @c In SICStus 3.12.0 @code{bdb:import/[2,3]} is available instead of
%@  @c @code{db_import/[2,3]}.
:- db_import/3 is det.
db_import(DBName, Options, ImportFile) :-
    import(DBName, Options, ImportFile).

:- db_import/2 is documented_as(db_import/3).
:- db_import/2 is det.
