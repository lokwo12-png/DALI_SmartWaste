...
/* [PM] 4.4.2 Various ways to work around the slow fsync() on Linux (Ubuntu 18.04, ext4)

# The 4.4.1 behavior, using fsync() at close, takes forever:
$ touch library/perf/perf_common.c && make cflags_extra=-DPERF_COMMON_DO_FSYNC > /dev/null 2>&1 && time ./Utils/x86_64-linux-glibc2.27/bin/sicstus -DSP_SPTI_PATH=perf --goal 'halt.'
SICStus 4.4.1 (x86_64-linux-glibc2.27): Mon Aug 27 15:27:58 CEST 2018
Licensed to SICS
...
# Workaround 1: Disabling all fsync() calls, with the undocumented -DSPIO_NO_FSYNC=yes
$ touch library/perf/perf_common.c && make cflags_extra=-DPERF_COMMON_DO_FSYNC > /dev/null 2>&1 && time ./Utils/x86_64-linux-glibc2.27/bin/sicstus -DSP_SPTI_PATH=perf -DSPIO_NO_FSYNC=yes --goal 'halt.'
SICStus 4.4.1 (x86_64-linux-glibc2.27): Mon Aug 27 15:27:58 CEST 2018
Licensed to SICS
...
# Workaround 2: Disabling all fsync() calls, with the eatmydata Ubuntu utility
$ touch library/perf/perf_common.c && make cflags_extra=-DPERF_COMMON_DO_FSYNC > /dev/null 2>&1 && time eatmydata ./Utils/x86_64-linux-glibc2.27/bin/sicstus -DSP_SPTI_PATH=perf --goal 'halt.'
SICStus 4.4.1 (x86_64-linux-glibc2.27): Mon Aug 27 15:27:58 CEST 2018
Licensed to SICS
...
# Workaround 3: 4.4.2 will avoid fsync() in perf_common.c
$ touch library/perf/perf_common.c && make cflags_extra=-DPERF_COMMON_DO_FSYNC=0 > /dev/null 2>&1 && time ./Utils/x86_64-linux-glibc2.27/bin/sicstus -DSP_SPTI_PATH=perf --goal 'halt.'
SICStus 4.4.1 (x86_64-linux-glibc2.27): Mon Aug 27 15:27:58 CEST 2018
Licensed to SICS
                                                ...
                                                char const *dir)
{
  ...
  perf_t_buf *buf = NULL;
  spio_t_bits sp_fclose_options = SPIO_OPTION_NONE;
  int retries = 0;
  char *map_path = NULL;
...
#if !PERF_COMMON_DO_FSYNC
  {
    /* [PM] 4.4.2 fsync() is _very_ slow in some Linux
       configurations. We do not care much about the written data, so
       suppress it. SPRM-20262. */

    sp_fclose_options |= SP_FCLOSE_OPTION_NO_FSYNC;
  }
#endif	/* !PERF_COMMON_DO_FSYNC */

  CHECK(ensdir(dir, mode));

  snprintf(pid_as_string, sizeof pid_as_string, "%lu", (unsigned long) pid);

#if !FORCE_BUILD && SICSTUS_VERSION > 40600 /* [PM] 4.3.4 postpone */
#error "Clean up file/dir creation (do it once, at init time), etc."
  ...
  */
  {
    ...
    CHECK(SP_fclose(map_stream, sp_fclose_options));
    map_stream = NULL;
  }
 ...
 retry:  
  ...
  CHECK(SP_fclose(stream, sp_fclose_options));
