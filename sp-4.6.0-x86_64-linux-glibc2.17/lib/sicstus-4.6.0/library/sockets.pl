...
%@  atom, something like @code{'sicstus.sics.se'} or
...
%% %@  The @var{Host} should be an atom, e.g.@: @code{'sicstus.sics.se'}. The
...
%@  @end table
%@  
%@  To create a binary stream to some web server @code{sicstus.sics.se}, you
%@  would do e.g.@:
%@  @example
%@  @group
...
:- socket_server_open/3 is det.
:- socket_server_open/3 is documented_as(socket_server_open/2).
