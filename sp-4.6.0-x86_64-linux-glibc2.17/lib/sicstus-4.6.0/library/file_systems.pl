...
%%      not help, in general).
%%
%%      For this reason all predicates that do access control
...
%@  @end table
%@  @end table
:- delete_directory/1 is det.
delete_directory(Directory) :-
	Goal = delete_directory(Directory),
	do_delete_directory(Directory, error, Goal).

:- delete_directory/2 is documented_as(delete_directory/1).
:- delete_directory/2 is det.
...
:- directory_exists/2 is documented_as(directory_exists/1).
:- directory_exists/2 is semidet.
...
:- file_exists/2 is documented_as(file_exists/1).
...
:- file_must_exist/2 is documented_as(file_must_exist/1).
:- file_must_exist/2 is det.
...
:- directory_must_exist/2 is documented_as(directory_must_exist/1).
:- directory_must_exist/2 is det.
...
%@  
%@  This uses @code{absolute_file_name/3} with the @code{glob/1} option.
:- directory_members_of_directory/1 is det.
...
%@  
%@  This uses @code{absolute_file_name/3} with the @code{glob/1} option.
:- directory_members_of_directory/2 is det.
...
%@  
%@  This uses @code{absolute_file_name/3} with a @code{glob(@var{Pattern})} option.
:- directory_members_of_directory/3 is det.
...
%@  
%@  This uses @code{absolute_file_name/3} with the @code{glob/1} option.
:- file_members_of_directory/1 is det.
...
%@  
%@  This uses @code{absolute_file_name/3} with the @code{glob/1} option.
:- file_members_of_directory/2 is det.
...
%@  
%@  This uses @code{absolute_file_name/3} with a @code{glob(@var{Pattern})} option.
:- file_members_of_directory/3 is det.
...
:- current_directory/1 is det.
current_directory(Dir) :-
	prolog:'$unix_cd'(Dir, Dir).


:- current_directory/2 is documented_as(current_directory/1).
:- current_directory/2 is det.
