...
fzn_run_stream(Stream, Options, GoalArg) :-
	...
	(   memberchk(solutions(Solutions), Options1) ->
	    true
	;   Solutions = 1
	),
	(   memberchk(search(Search), Options1) ->
	    true
	;   Search = bab
	),
...
fzn_constraint_clpfd(fzn_member_bool, [[X]-M, Y], Anns,
		     [Index in 1..M, element(Index, X, Y)], [array(M, int), int], Anns) :-
	Index in 1..M.
