...
other_msg(statistics(Props)) -->
	...
	['memory (total) ~t~d~28| bytes'-[MemoryTotal],nl,
	 ...
	 '   choice stack ~t~d~28| bytes:~t~d~18+ in use, ~t~d~18+ free'-
...
profile_line(nondet(Part), TTotal, Callee) -->
	{Ind = '%' /*nondet_indicator(Callee, Ind)*/},
	{arg(1, TTotal, Total)},
	['~t~w~d~36|/~d~t~48|~q'-[Ind,Part,Total,Callee],nl].
...
profile_line(nondet(Calls), Insns, Choices, Self) -->
	{nondet_indicator(Self, Ind)},
	['~t~d~12|~t~d~24|~t~w~d~36|~t~44|~q'-[Insns,Choices,Ind,Calls,Self],nl].

nondet_indicator(Module:Name/Arity, Ind) :-
	functor(Functor, Name, Arity),
	prolog:current_is_directive(Functor, Module, Is, _, _, _),
	(   Is = nondet -> Ind = '%'
	;   Is = det -> Ind = '!'
	;   Is = semidet -> Ind = '!'
	),
