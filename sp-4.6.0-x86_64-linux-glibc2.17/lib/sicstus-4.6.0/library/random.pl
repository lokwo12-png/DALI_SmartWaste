...
%@
%@  The random generator starts with the same default initial state on each run. See @code{setrand/1}
...
%@  raises an error.
%@
%@  The initial state of the random number generatior is always the same.
